...
import com.iuh.HP_toystore.repository.ChiTietTimKiemRepository;
import com.iuh.HP_toystore.repository.DanhMucRepository;
import com.iuh.HP_toystore.repository.NguoiDungRepository;
import com.iuh.HP_toystore.repository.TaiKhoanRepository;
import com.iuh.HP_toystore.repository.TimKiemRepository;
import com.iuh.HP_toystore.services.IDanhMucService;
import com.iuh.HP_toystore.services.INguoiDungService;
	...
	public String listSanPham(Model model) {
		...
		if (taiKhoan != null) {
			...
			List<ChiTietTimKiem> listctChiTietTimKiems = chiTietTimKiemRepository
					.findAllChiTietTimKiemsByNguoiDungAndThoiGianTimKiemBetweenAndTrangThai(nguoiDung, start, end,"Có");
			...
			if (listctChiTietTimKiems != null && !listctChiTietTimKiems.isEmpty()) {
				...
				System.err.println(cttk.getTimKiem().getKeyWord());

				model.addAttribute("listSanPhamDanhMuc",
						sanPhamService.timKIemMoi(pageable1, cttk.getTimKiem().getKeyWord()));
				...
				Page<SanPham> pageSp = sanPhamService.timKIemMoi(pageable1, cttk.getTimKiem().getKeyWord());

				
//				int dem = 2;
//				for (int i = pageSp.getTotalPages(); i < 12; i++) {
//					ChiTietTimKiem cttkp = listctChiTietTimKiems.get(listctChiTietTimKiems.size() - dem);
//					dem = dem + 1;
//					
//					Page<SanPham> pageSpp = sanPhamService.timKIemMoi(pageable1, cttkp.getTimKiem().getKeyWord());
//					
//
//				}
				

				model.addAttribute("listSanPham", pageSp);
		...
		System.out.println(sanPhamService.findAllSanPhamsByTrangThai(pageable1, "Đã Xác Nhận"));
		return "trang-chu";
	}
		...
		if (taiKhoan != null) {
			...
			if (tkkw != null) {
				...
				if (ct == null) {
					...
					cts.setTimKiem(tkkw);
					cts.setTrangThai(tkkw.getTrangThai());
					cts.setSoLuong(1);
					...
					ct.setTrangThai(tkkw.getTrangThai());
					chiTietTimKiemRepository.save(ct);
				}

			} else {
				cttk.setTimKiem(timKiem);
				cttk.setTrangThai(timKiem.getTrangThai());
